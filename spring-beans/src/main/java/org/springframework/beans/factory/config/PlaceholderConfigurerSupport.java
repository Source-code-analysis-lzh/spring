...
/**
 * 属性资源配置器的抽象基类，用于解析bean定义属性值中的占位符。 实现将值从属性文件或其它
 * {@linkplain org.springframework.core.env.PropertySource 属性源}提取到Bean定义中。
 *
 ...
 *
 * 带注释的Bean定义可以使用{@link org.springframework.beans.factory.annotation.Value @Value}
 * 注释来利用属性替换的优势：
 *
 * <pre class="code">@Value("${person.age}")</pre>
 *
 * 实现检查bean引用中的简单属性值，列表，map，props和bean名称。 此外，占位符值还可以交叉引用其它占位符，例如：
 *
 ...
 *
 * <p>如果配置器无法解析占位符，则将引发{@link BeanDefinitionStoreException}。 如果要检查多个属性文件，
 * 请通过{@link #setLocations locations}属性指定多个资源。 您还可以定义多个配置器，每个配置器都有自己的占位符语法。 
 * 如果无法解析占位符，请使用{@link #ignoreUnresolvablePlaceholders}故意抑制引发异常。
 *
 * <p>可以通过{@link #setProperties properties}属性为每个配置器实例全局定义默认属性值，
 * 也可以使用默认值分隔符（默认情况下为{@code ":"}可通过{@link #setValueSeparator(String) 进行自定义）
 * 在逐个属性的基础上进行定义。
 *
