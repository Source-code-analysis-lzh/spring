 ...
 * 可以检查传入的ResourceLoader（例如，在上下文中运行时通过
 * {@link org.springframework.context.ResourceLoaderAware}传入的
 * {@link org.springframework.context.ApplicationContext}）是否也实现了此扩展接口.
 *
 * <p>{@link PathMatchingResourcePatternResolver}是一个独立的实现，
 * 可在ApplicationContext外部使用，{@link ResourceArrayPropertyEditor}也使用它来填充Resource数组Bean属性.
 *
...
public interface ResourcePatternResolver extends ResourceLoader {
	 ...
	 * 在于它检索给定名称（例如"/beans.xml"）的所有匹配资源，例如在根目录中所有已部署的JAR文件.
	 ...
	 * <p>应尽可能避免指向相同物理资源的资源条目重叠. 结果应具有设定的语义.
